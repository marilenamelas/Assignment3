...
```{r}
b<-ggplot(diamonds, aes(x = carat, y = price, color=clarity)) + geom_point() + facet_wrap(~cut) + ggtitle("Plot of carat vs. price")
example6<-ggplotly(b)
return(b)
```
...
inputPanel(
  ...
  selectInput("var4",  label = "Choose Yaxis:", choices = c("depth","carat"), selected = "carat")
)
```
...
renderPlotly({
  ...
  y_axis <- input$var4
  clarity <- "clarity"
   a<-ggplot(diamonds, aes_string(x = x_axis, y = y_axis, color=clarity)) + geom_point() + facet_wrap(~cut) + ggtitle("Plot of cut vs. price")
   ggplotly(a)
}) 
```
