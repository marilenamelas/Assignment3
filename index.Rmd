...
+ *ncol(iris) : 5 columns*
*names(iris) : "Sepal.Length" "Sepal.Width" "Petal.Lenght" "Petal.Width" "Species".*
```{r}
mean_petal_length <- mean(iris$'Petal.Length')
mean_petal_length
...
```{r echo = FALSE}
inputPanel(
  selectInput("var1", label = "Choose Xaxis:", choices = c("depth","price"), selected = "price"),
  selectInput("var2",  label = "Choose Yaxis:", choices = c("carat","depth"), selected = "depth")
)
...
renderPlotly({
  ...
  y_axis <- "depth"
  clarity <- "clarity"
   a<-ggplot(diamonds, aes_string(x = x_axis, y = y_axis, color = clarity)) + 
    geom_point() + facet_wrap(~cut) + 
    ggtitle("Plot of depth vs. price")
   ggplotly(a)
}) 
```
